...
async function main() {
    ...
    // Categories
    const catTech = await prisma.category.create({ data: { name: "Technologie" } });
    const catTravel = await prisma.category.create({ data: { name: "Cestování" } });
    const catCulture = await prisma.category.create({ data: { name: "Kultura" } });
    ...
    const article = await prisma.article.create({
        data: {
            ...
            published: true,
            authorId: user.id,
            categories: { connect: [{ id: catCulture.id }] },
        },
    });
